...
describe('Ticket model', () => {
	each([
		...
		"Customer can enter only allowed swimming pools ( input is '%s' '%s' '%s' )",
		async (allowedSwimmingPools, testSwimmingPool, expected) => {
	...
	each([
		...
		"Customer can enter only between valid dates ( input is '%s' '%s' '%s' '%s' )",
		async (validFrom, validTo, testTime, expected) => {
			let ticket = new TicketModel()
			ticket.ticketType = new TicketTypeModel({
				validFrom: validFrom,
				validTo: validTo,
	...
	each([
		...
		"Test enough remaining entries ( input is '%s' '%s' '%s' '%s' )",
		async (ticketType, remainingEntries, entry, expected) => {
			let ticket = new TicketModel()
			ticket.ticketType = new TicketTypeModel()
	...
	each([
		...
		['17:00:00', '23:59:00', true, '2021-05-12 16:59:59', false],
		['17:00:00', '23:59:00', true, '2021-05-12 17:00:00', true],
		['17:00:00', '23:59:00', true, '2021-05-12 23:00:00', true],
		['17:00:00', '23:59:00', true, '2021-05-12 00:00:00', false],
	...
	each([
		[
			true,
			'02:15:00',
			new EntryModel({ timestamp: '2021-05-03 15:55:10' }),
			'2021-05-03 15:55:10',
			true,
		],
		[
