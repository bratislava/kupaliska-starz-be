...
describe(`[DELETE] ${endpoint})`, () => {
	...
	it('Response should return code 200', async () => {
		...
		const user = await UserModel.findByPk(response.body.data.id, { paranoid: false });
	...
	it('CANNOT delete SUPER_ADMIN', async () => {
		let response = await request.delete(endpoint(superAdminToBeDeletedId))
			.set('Content-Type', 'application/json')
			.set('Authorization', `Bearer ${process.env.jwtSuperAdmin}`)

		expect(response.status).toBe(403)

		response = await request.delete(endpoint(superAdminToBeDeletedId))
		.set('Content-Type', 'application/json')
		.set('Authorization', `Bearer ${process.env.jwtOperator}`)
	...
	it('OPERATOR CANNOT delete another OPERATOR', async () => {
		const response = await request.delete(endpoint(operatorToBeDeletedId))
			.set('Content-Type', 'application/json')
			.set('Authorization', `Bearer ${process.env.jwtOperator}`)
