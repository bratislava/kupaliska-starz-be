...
describe(`[GET] ${endpoint})`, () => {
	...
	it('Expect status 404', async () => {
		const response = await request.get(endpoint(uuidv4()))
			.set('Content-Type', 'application/json')
	...
	it('Response should return code 200', async () => {
		...
		expect(response.body.openingHours).toStrictEqual( [{ startFrom: '2021-01-01', startTo: '2022-01-01' }])
	...
	it('Can get only assigned swimming pool', async () => {
		...
		response = await request.get(endpoint(swimmingPool2Id))
			.set('Content-Type', 'application/json')
			.set('Authorization', `Bearer ${process.env.jwtSwimmingPoolOperator}`)
		expect(response.status).toBe(403)
	})
