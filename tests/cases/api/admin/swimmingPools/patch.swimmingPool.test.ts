...
describe(`[PATCH] ${endpoint})`, () => {
	...
	beforeAll(async () => {
		...
		await FileModel.bulkCreate([
			createFile(swimmingPoolId)
		])

		await writeFile(path.join(appConfig.filesPath, 'public/swimming-pools/filename-test.png'), 'image')
	...
	it('Operator CANOT patch other swimming pool', async () => {
		const response = await request.patch(endpoint(swimmingPool2Id))
			.set('Content-Type', 'application/json')
			.set('Authorization', `Bearer ${process.env.jwtSwimmingPoolOperator}`)
			.send({
				waterTemp: 0,
