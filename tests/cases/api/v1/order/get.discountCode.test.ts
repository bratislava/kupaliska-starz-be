...
const endpoint = (discountCode: string, ticketTypeId: string) => `/api/v1/orders/discountCodes/${discountCode}/ticketTypes/${ticketTypeId}`
...
describe(`[GET] ${endpoint}`, () => {
	...
	it('Discount code not found', async () => {
		...
		const response = await request.get(endpoint(invalidDiscountCode, 'c70954c7-970d-4f1a-acf4-12b91acabe06'))
	...
	it('Discount code not between valid dates', async () => {
		...
		let response = await request.get(endpoint(discountCode, 'c70954c7-970d-4f1a-acf4-12b91acabe06'))
			.set('Content-Type', 'application/json')
		...
		response = await request.get(endpoint(discountCode, 'c70954c7-970d-4f1a-acf4-12b91acabe06'))
			.set('Content-Type', 'application/json')
	...
	it('Discount code already used', async () => {
		...
		const response = await request.get(endpoint(discountCodeUsedAt, 'c70954c7-970d-4f1a-acf4-12b91acabe06'))
			.set('Content-Type', 'application/json')
	...
	it('Bad ticket type', async () => {
		...
		const response = await request.get(endpoint(discountCode, uuidv4()))
			.set('Content-Type', 'application/json')
