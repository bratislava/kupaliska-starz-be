...
let turnstile:
	| ((token: string, options?: TurnstileOptions) => Promise<TurnstileResponse>)
	| undefined

if (!process.env.TURNSTILE_SECRET_KEY) {
	logger.warn('TURNSTILE_SECRET not set! Using dummy token, captcha will always pass.')
...
export default async (req: Request, _res: Response, next: NextFunction) => {
	try {
		...
		if (!isDefined(token)) {
			throw new ErrorBuilder(400, req.t('error:missingRecaptchaToken'))
		}
