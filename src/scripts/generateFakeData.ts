...
async function generateData() {
	...
	for (let i = 0; i <= 20; i++) { // 20 iteration, in every iteration will be created (numberOfOrdersInBulk x 2) orders
			...
			map([...Array(numberOfOrdersInBulk).keys()], () => {
				...
				return {
					...
					tickets: concat([{
						...
						entries: generateEntries(numberOfEntries, eventsFrom, eventsTo, swimmingPoolsIds, employee.id)
					}],
		...
		await Order.bulkCreate(
			...
			map([...Array(numberOfOrdersInBulk).keys()], () => {
				...
				const addDiscount = faker.random.boolean()
				const price = (entriesTicketType.price) * (addDiscount ? ((100 - discountCode.amount) / 100) : 1)

				return {
					...createOrder(),
					price: price,
					discount: addDiscount ? (entriesTicketType.price) * ((discountCode.amount) / 100) : 0,
					discountCodeId: addDiscount ? discountCode.id : null,
...
const generateEntries = (numberOfEntries:number, eventsFrom:string, eventsTo:string, swimmingPoolsIds:any, employeeId:string) => map([...Array(numberOfEntries).keys()], () => {
