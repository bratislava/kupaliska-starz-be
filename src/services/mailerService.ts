...
export const sendEmail = async (
	...
	try {
		...
		captureError(err, req.ip, 'emailData', { from: mailData.from, to: recipient, variables, emailType: 'templateEmail' })
		throw new ErrorBuilder(424, i18next.t('error:emailFailed'))
	}
}

export const sendRawEmail = async (req: Request, recipient: string, subject: string, html: string): Promise<void> => {
	...
	try {
		...
		logger.error(`${424} - EMAIL ERROR - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`)
		captureError(err, req.ip, 'emailData', { from: mailData.from, to: recipient, emailType: 'rawEmail'})
		throw new ErrorBuilder(424, i18next.t('error:emailFailed'))
	}
}
