...
export const discountCodeAddSchema = {
	...
	ticketTypes: Joi.array().min(1).required().items(
...
export const workflow = async (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		const existingCodes = map(await DiscountCode.findAll({
			paranoid: false,
			attributes: ['code']
		}), (code) => (code.code))

		const discountCodes = await DiscountCode.bulkCreate(map([...Array(body.quantity).keys()], () => {
		...
		await DiscountCodeTicketTypeModel.bulkCreate(discountCodeTicketTypes, { transaction })
		...
		return res.json({
			data: {
				discountCodes: map(createdDiscountCodes, (discountCode) => (formatDiscountCode(discountCode))),
			},
			messages: [{
