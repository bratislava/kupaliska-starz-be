...
export const workflow = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const swimmingPoolsResult = await sequelize.query<{ id: string, range: { age: string, value: string } }>(`
		...
		const swimmingPoolsAverageUniqueVisitsResult = await sequelize.query<{ id: string, total: string }>(`
		...
		const agesAverageUniqueVisitsResult = await sequelize.query<{ age: string, total: string }>(`
		...
		return res.json({
			data: {
				swimmingPools: formatVisits(allSwimmingPools, allAges, swimmingPoolsVisits, swimmingPoolsAverageUniqueVisitsResult),
				ages: reduce(agesAverageUniqueVisitsResult, (allRanges, range) => {
