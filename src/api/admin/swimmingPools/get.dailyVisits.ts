...
export const workflow = async (
	...
	try {
		...
		const buckets = map(range(0, numberOfBuckets), () =>
			query.intervalDividedBy ? ({} as any) : 0
		)
		const tempBuckets = map(range(0, numberOfBuckets), () =>
			query.intervalDividedBy ? ({} as any) : 0
		)
