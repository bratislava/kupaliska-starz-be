...
export const workflow = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const [filterVariables, filterSQL] = getFilters( { customerEmail: customerEmail, customerNames: customerName })
		...
		if (age) {
			...
			ageFilterSql = age.showUnspecified ? `(${ageFilterSql} OR "customerAges" IS NULL)` : ageFilterSql
		...
		if (user.role === USER_ROLE.SWIMMING_POOL_OPERATOR) {
            ...
            const usersSwimmingPoolsArray = map(usersSwimmingPools, (_id, index) => (`$swimmingPool${index}::uuid`)).join(',')
		...
		return res.json({
			customers: map(customers, (customer) => ({
                customerEmail: user.role === USER_ROLE.SWIMMING_POOL_OPERATOR ? '*****' :customer.customerEmail,
				customerAges: customer.customerAges,
				customerZips: customer.customerZips,
				customerNames: user.role === USER_ROLE.SWIMMING_POOL_OPERATOR ? '*****' :customer.customerNames,
