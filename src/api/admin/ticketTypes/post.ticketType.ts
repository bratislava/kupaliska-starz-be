...
export const ticketTypeAddSchema = {
	...
	type: Joi.string().uppercase().valid(...TICKET_TYPES).required(),
	...
	childrenMaxNumber: Joi.number().min(0).max(100)
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	childrenPrice: Joi.number().min(0).precision(2)
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	childrenAgeFrom: Joi.number().min(0).max(150)
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	childrenAgeTo: Joi.number().min(0).max(150)
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	childrenAgeToWithAdult: Joi.number().min(0).max(150)
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	childrenPhotoRequired: Joi.boolean()
		.when('childrenAllowed', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	validFrom: Joi.date().required(),
	validTo: Joi.date().min(Joi.ref('validFrom')).required(),
	entriesNumber: Joi.number().min(0).max(1000)
		.when('type', { is: Joi.valid(TICKET_TYPE.ENTRIES), then: Joi.required(), otherwise: Joi.forbidden() }),
	hasTicketDuration: Joi.boolean().required().default(false)
		.when('hasEntranceConstraints', { is: true, then: Joi.boolean().valid(false)}),
	ticketDuration: JoiExtended.time()
		.when('hasTicketDuration', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	hasEntranceConstraints: Joi.boolean().required().default(false),
	entranceFrom: JoiExtended.time()
		.when('hasEntranceConstraints', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
	entranceTo: JoiExtended.time().minTime(Joi.ref('entranceFrom'))
		.when('hasEntranceConstraints', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
}
