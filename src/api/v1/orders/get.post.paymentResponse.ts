...
import { webpayLogger as logger } from '../../../utils/logger'
...
export const workflow = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		logger.info(`INFO - Payment response received - ${JSON.stringify(data)} - ${JSON.stringify(req.method)} - ${req.ip}`)
		...
		if (!data.DIGEST || !data.DIGEST1) {
			logger.info(`ERROR - ${400} - DIGEST OR DIGEST1 not provided - ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
			return res.redirect(`${webpayConfig.clientAppUrl}/order-result?success=false`)
		}

		if (!data.PRCODE || !data.SRCODE) {
			logger.info(`ERROR - ${400} - PRCODE or SRCODE not provided - ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
			return res.redirect(`${webpayConfig.clientAppUrl}/order-result?success=false`)
		}

		if (!order) {
			logger.info(`ERROR - ${404} - Order not found - ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
			return res.redirect(`${webpayConfig.clientAppUrl}/order-result?success=false`)
		}
		...
		if (!paymentOrder) {
			logger.info(`ERROR - ${404} - Payment order not found - ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
		...
		if (!paymentResult.isVerified) {
			logger.info(`ERROR - ${400} - Payment verification failed - ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
		...
		if (!paymentResult.isSuccess) {
			logger.info(`ERROR - ${400} - Payment was not successful- ${JSON.stringify(data)} - ${req.method} - ${req.ip}`)
		...
		return res.redirect(`${webpayConfig.clientAppUrl}/order-result?${queryParams}`)
	} catch (error) {
		return next(error)
	}
}
