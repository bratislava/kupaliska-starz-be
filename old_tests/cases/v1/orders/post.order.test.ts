...
const clearAllFiles = async () => {
	const files = await readdir(
		path.join(appConfig.filesPath, 'private/profile-photos')
	)
	for (const file of files) {
		await unlink(
			path.join(appConfig.filesPath, 'private/profile-photos', file)
		)
	}
}
...
describe('[POST] /api/v1/orders', () => {
	...
	describe('base validation tests', () => {
					...
					tickets: [
						{
							quantity: 1,
							ticketTypeId:
								'c70954c7-970d-4f1a-acf4-12b91acabe03',
							age: 18,
							zip: '03251',
							email: faker.internet.email(),
						},
		...
		it('Max number of the tickets exceeded', async () => {
			const response = await request
				...
				.send({
					tickets: [
						{
							quantity:
								Number(appConfig.maxTicketPurchaseLimit) + 1,
							ticketTypeId:
								'c70954c7-970d-4f1a-acf4-12b91acabe03',
							age: 18,
							zip: '03251',
			...
			expect(response.status).toBe(400)
			expect(response.body.messages[0].path).toBe(
				'body.tickets.0.quantity'
			)
		})
		...
		each([
			...
			"Children`s age must be in the range when the input is '%s' (dependent on the ticket type)",
			async (age, expected) => {
