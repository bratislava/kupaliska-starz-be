...
const privateKeyPath = path.join(process.env.GP_WEBPAY_KEYS_PATH, 'merchant-pvk.key')
const gpPublicKeyPath = path.join(process.env.GP_WEBPAY_KEYS_PATH, 'gpe.signing.pem')

// eslint-disable-next-line no-unused-expressions
fs.existsSync(keysPath) || fs.mkdirSync(keysPath, { recursive: true })
// eslint-disable-next-line no-unused-expressions
fs.existsSync(privateKeyPath) || fs.writeFileSync(privateKeyPath, 'private-key-test')
// eslint-disable-next-line no-unused-expressions
fs.existsSync(gpPublicKeyPath) || fs.writeFileSync(gpPublicKeyPath, 'public-key-test')
